...
let p1 = d3.select("#hover");
let p2 = d3.select("#img");
let p3 = d3.select("#name");
let p4 = d3.select("#neighborhood");
let p5 = d3.select("#keyIssues");
let p6 = d3.select("#learnMore");
let p7 = d3.select("#name");
...
d3.json("Javascript/district.geojson", function(error, data) {
    ...
    map.selectAll("path")
        ...
        .on("mouseover", function(d, i) {
            ...
            p2.attr("src", tooltipInfo[d.properties.DISTRICT].Gray.img);
            p3.html(`${tooltipInfo[d.properties.DISTRICT].Gray.name}`);
            p3.html(`Candidate's home neighborhood: ${tooltipInfo[d.properties.DISTRICT].Gray.neighborhood}`);
            p5.html(`Key platform issue${tooltipInfo[d.properties.DISTRICT].Gray.bio}`);
            p6.attr("href", "https://thescopeboston.org/");
